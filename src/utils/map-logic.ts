...
export function init({
  ...
  // --- Map Initialization ---
  const map = L.map("map").setView([-8.21, 114.37], 13); // Default view of Banyuwangi
  ...
  let selectedRouteLayer: (L.Layer & { originalStyle?: L.PathOptions }) | null =
    null;
  let findPointMarker: L.Marker | null = null;
  map.on("click", () => {
    if (selectedRouteLayer) {
      (selectedRouteLayer as L.Path).setStyle(
  ...
  if (routes && routes.length > 0) {
    routes.forEach((routeData) => {
      if (routeData && routeData.geometry) {
        ...
        const conn = connections.find((c) => c.id === routeData.connectionId);
        ...
        const routeLayer = L.geoJSON(routeData.geometry, {
          ...
          onEachFeature: function (feature, layer) {
            ...
            layer.on("click", (e) => {
              ...
              selectedRouteLayer = layer as L.Layer & {
  ...
  const deviceASelect = document.getElementById(
    "deviceAId",
  ) as HTMLSelectElement;
  const portASelect = document.getElementById("portAId") as HTMLSelectElement;
  const deviceBSelect = document.getElementById(
    "deviceBId",
  ) as HTMLSelectElement;
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(`Failed to ${isEdit ? "update" : "save"} connection`);
