<script>
    ...
    eventSource.addEventListener("notification", (event) => {
      ...
      if (data.nodeChanges) {
        for (const change of data.nodeChanges) {
          if (!updates.has(change.ipMgmt)) {
            updates.set(change.ipMgmt, { node: {}, interfaces: new Map() });
          }
      ...
      if (data.interfaceChanges) {
        for (const change of data.interfaceChanges) {
          ...
          updates.get(change.ipMgmt).interfaces.set(change.ifIndex, change);
        }
      }
      ...
      for (const [
        ...
        const nodeIndex = allNodes.findIndex((n) => n.ipMgmt === ipMgmt);
  ...
  const filteredNodes = $derived(
    currentFilter ? allNodes.filter((node) => node.status !== 1) : allNodes
  );

  const onlineCount = $derived(
    allNodes.filter((node) => node.status === 1).length
  );
